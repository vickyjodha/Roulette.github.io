(function($) {
	var Roulette = function(options) {
		...
		var slowDownSetup = function() {
			...
			var bbb=	p.maxDistance += (p.totalHeight - (p.maxDistance % p.totalHeight) + (p.stopImageNumber * p.imageHeight))
                    % p.totalHeight;
                console.log(bbb)
			}
		}
		...
		var roll = function() {
			...
			if (p.maxDistance && p.distance >= p.maxDistance) {
                ...
                console.log(p);
              var hghj=  p.stopCallback(p.$rouletteTarget.find('img').eq(p.stopImageNumber));
                console.log(p.$rouletteTarget.find('img'));
				return;
			}
			p.distance += speed_;
